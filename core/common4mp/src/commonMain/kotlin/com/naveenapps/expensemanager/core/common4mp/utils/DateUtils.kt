...
fun Long.toExactStartOfTheDay(): LocalDate {
    // First, determine the date by interpreting the timestamp as a UTC instant
    val dateInUtc = Instant.fromEpochMilliseconds(this)
        .toLocalDateTime(TimeZone.UTC)
        .date

    // Then, find the start of that calendar date in the system's default timezone
    return dateInUtc.atStartOfDayIn(TimeZone.currentSystemDefault())
